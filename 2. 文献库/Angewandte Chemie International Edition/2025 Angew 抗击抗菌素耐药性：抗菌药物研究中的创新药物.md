![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=M2QzYjdkMDk5MWI3ZTQxZmMxZTRhMGRkZDNkNWE1NGJfS21uY0U0ZDZadkFxaXJEVGNkMldHWm1oQXc1MUk5bU5fVG9rZW46WVJFWGJETHFMb2RFZjh4eHpXeGNlMktEblhiXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

# 一：文献基本信息

**文章题目**: Fighting Antimicrobial Resistance: Innovative Drugs in Antibacterial Research

**文章** **DOI** **号**: 10.1002/anie.202414325

**期刊名称**: _Angewandte Chemie International Edition_

**通讯作者**: Roderich D. Süssmuth

**通讯作者工作单位**: 德国柏林工业大学化学研究所 (Institut für Chemie, Technische Universität Berlin, Germany)

# 二：核心摘要

本综述系统性地、全景式地描绘了当前全球在应对**抗菌素****耐药性** **(Antimicrobial Resistance, AMR)** 危机背景下，新型抗菌药物发现与开发的完整图景。

文章的核心论点是，**随着大型****制药公司****普遍退出该领域，抗菌药物的研发重心已不可逆转地转移至学术机构和中小型企业，这不仅对研发管线造成了严重冲击，也催生了对创新发现策略和全新合作模式的迫切需求**。

综述首先界定了当前面临的主要威胁——以 **ESKAPE** 病原体为代表的多重耐药菌，并阐述了理想抗菌药物所需具备的关键特性，包括靶点选择、耐药性规避和药物动力学考量。在此基础上，文章深入探讨了现代药物发现的三大支柱：源于自然的传统筛选与基因组挖掘、基于化学合成的理性设计与库筛选，以及新兴的人工智能辅助发现。尤为关键的是，本综述的后半部分以靶点为核心，对当前处于研发管线中的关键候选药物进行了百科全书式的汇编和评述，系统性地覆盖了从经典的细胞壁合成、蛋白质合成、DNA/RNA 合成，到新兴的脂肪酸合成、蛋白酶、膜生物发生等靶点。通过对每种候选分子的来源、抗菌谱、耐药机制和开发状态的详细剖析，本文不仅为药物化学和微生物学领域的研究者提供了一份宝贵的、数据详实的参考资料，也为未来设计能够克服耐药性挑战的新一代抗菌药物指明了战略方向。

# 三：结构与逻辑

本综述的论证逻辑遵循一个从“问题”到“策略”再到“解决方案实例”的宏大框架。

1. **问题陈述 (The Problem)**：首先，文章界定了 AMR 危机的严峻性，明确了关键的病原体、理想的药物靶点和耐药机制，并勾勒出药物开发流程中的挑战。
    
2. **策略总览 (The Strategies)**：其次，文章系统性地梳理了当前用于发现新抗菌药物的各种策略，涵盖了从自然、化学合成到人工智能的全部现代方法学。
    
3. **解决方案实例 (The Solutions in Practice)**：最后，也是本文的核心部分，文章以靶点为纲，详细列举和评述了当前处于不同研发阶段的、最具代表性的创新药物分子，作为前述策略的成果展示。
    

**引言：AMR 危机与断裂的抗菌药物研发管线**

**第一部分：抗菌药物研发的战场与规则**

- 1.1 关键病原体：我们面对的敌人
    
- 1.2 理想靶点与药物特性：制胜的法则
    
- 1.3 细菌的防御：耐药机制的解析与反制
    
- 1.4 药物开发流程：从命中到候选的漫漫长路
    

**第二部分：新武器的来源与发现策略**

- 2.1 源于自然的发现：经典的宝库与现代的挖掘
    
- 2.2 化学与化学生物学：从头构建新分子
    
- 2.3 计算科学的赋能：分子对接与人工智能
    
- 2.4 设计理念的革新：双效机制与先导优化
    

**第三部分：创新药物管线全景：一场靶点导向的阅兵**

- 3.1 细胞壁与细胞分裂：攻破细菌的“铠甲”
    
- 3.2 蛋白质合成：瘫痪细菌的“工厂”
    
- 3.3 DNA 合成：阻断遗传信息的复制
    
- 3.4 RNA 合成：沉默基因的转录
    
- 3.5 脂肪酸合成：切断膜组分的来源
    
- 3.6 蛋白酶与降解机器：破坏蛋白质稳态
    
- 3.7 细胞膜与生物能学：瓦解能量与屏障
    
- 3.8 膜蛋白与组分生物发生：干扰“城墙”的建造
    
- 3.9 其他新兴靶点：探索新的攻击路径
    

**第四部分：总结与展望**

---

# 第一部分：抗菌药物研发的战场与规则

## 1.1 关键病原体：我们面对的敌人

抗生素研发的首要任务是明确目标病原体。世界卫生组织 (WHO) 和美国疾病控制与预防中心 (CDC) 均发布了需要优先应对的耐药病原体清单。其中，**ESKAPE/ESKAPEE** 病原体组合是一个广为接受的、囊括了主要院内感染“超级细菌”的缩写，包括：

- **屎肠球菌 (Enterococcus faecium, Ef)**
    
- **金黄色葡萄球菌 (Staphylococcus aureus, Sa)**
    
- **肺炎克雷伯菌 (Klebsiella pneumoniae, Kp)**
    
- **鲍曼不动杆菌 (Acinetobacter baumannii, Ab)**
    
- **铜绿假单胞菌 (Pseudomonas aeruginosa, Pa)**
    
- **肠杆菌属 (Enterobacter species, Eb)**
    
- **大肠杆菌 (Escherichia coli, Ec)**
    

这些病原体，特别是革兰氏阴性菌（Kp, Ab, Pa, Eb, Ec），由于其**外膜 (Outer Membrane)** 结构提供了一道额外的物理和化学屏障，使得药物渗透极为困难，构成了当前最严峻的治疗挑战。

除了 ESKAPE 组合，其他重要的病原体还包括引起性传播疾病的**淋病奈瑟菌 (Neisseria gonorrhoeae)**、引起艰难梭菌相关性腹泻的**艰难梭菌 (Clostridioides difficile)**，以及需要特殊长程治疗的**结核分枝杆菌 (Mycobacterium tuberculosis, Mt)**。这些细菌的生活方式和生理特性各异，例如结核菌的蜡质细胞壁和胞内寄生特性，都对药物设计提出了特殊要求。

此外，细菌的致病性不仅与其生长有关，还与其产生的**毒力因子 (Virulence factors)** 密切相关。这些毒力因子（如毒素、粘附素、分泌系统）通常不是细菌生存所必需的，但却是致病的关键。因此，靶向毒力因子或其调控系统（如**群体感应 (Quorum Sensing, QS)**）的**抗毒力疗法**，旨在“解除细菌武装”而非直接杀死它们，被认为是一种可能减缓耐药性产生的策略。然而，其在急性感染中的疗效和适用性仍在争论中。

许多病原体还能形成**生物膜 (Biofilms)** [**图 2 (示意图)**]，这是一种由细菌群体分泌的胞外基质包裹形成的复杂结构。生物膜内的细菌代谢缓慢，对传统抗生素的敏感性可降低数百倍，是导致慢性感染和植入物相关感染的主要原因。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=YzVkNzM0MzY5ZGVmZDgxMDc0OGU0ZmFjOGM2OWJlYzBfb1VFOHZrVkVERzdnQ200WXpFeXFIbWt4NEswSHJ6NWVfVG9rZW46Snd4dGJUSFR0b0J4VWV4djBudmNJRVpvbmswXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

## 1.2 理想靶点与药物特性：制胜的法则

一个理想的抗菌药物靶点应具备以下特征：

1. **必需性 (Essentiality)**：对细菌的生长或存活至关重要。
    
2. **低耐药倾向**：靶点不易通过突变或旁路途径产生耐药性。
    
3. **可及性 (Accessibility)**：药物能够到达并结合靶点。
    
4. **选择性 (Selectivity)**：靶点在细菌中存在，但在人体中不存在或存在显著差异，以避免脱靶毒性。
    

[**图 1：已确立和新兴的抗菌靶点示意图**] 该图是理解抗菌药物作用机制的“战略地图”。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=NzRlZDE1ZDllMzk1NGI1NDI1YmY5MDlhOTAxYjc3NTdfendSSkREdTF5V1RvQXpENk16MHZCaFI2cjlqeERkNUhfVG9rZW46VkVhS2J0RW1ab2VDT0h4cHdGa2NZMEtlbkdnXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **描述性说明**：该图以革兰氏阴性菌、革兰氏阳性菌和分枝杆菌的细胞结构为背景，系统性地标示了从细胞壁、细胞膜到细胞质内各种已确立和新兴的药物靶点。
    
- **揭示性分析**：该图深刻地揭示了抗菌药物靶点的多样性和分布。**经典靶点**主要集中在几个核心的生物大分子合成通路上：(1) **细胞壁合成**（肽聚糖、壁磷壁酸、分枝菌酸）；(2) **蛋白质合成**（核糖体）；(3) **DNA 复制**（拓扑异构酶）；(4) **RNA 合成**（RNA 聚合酶）；(5) **叶酸代谢**。而**新兴靶点**则扩展到更精细的细胞过程中，如**脂肪酸合成**、**蛋白酶**、**细胞分裂 (FtsZ)**、**膜生物发生**（LPS 转运、β-桶状蛋白组装）和**生物能学**（ATP 合酶、呼吸链）。该图强调了革兰氏阴性菌外膜（OM）和分枝杆菌的特殊细胞壁结构是药物渗透的主要障碍，因此，靶向这些外层结构的组装和转运过程（如 Lpt、BAM 系统）是当前研发的热点。
    

药物的杀菌模式分为**杀菌性 (Bactericidal)** 和**抑菌性 (Bacteriostatic)**。杀菌药直接杀死细菌，而抑菌药仅抑制其生长，依赖宿主免疫系统清除病原体。虽然杀菌药在某些情况下更受青睐，但快速裂解细菌也可能释放大量内毒素，引发毒性休克。

一个关键的评价指标是**耐药频率 (Frequency of Resistance, FoR)**，即在药物选择压力下产生耐药突变体的概率。理想的药物应具有较低的 FoR。此外，**多靶点 (multi-target)** 或**多效性 (multi-effective)** 药物，即能作用于一个通路中的多个靶点或通过一个靶点影响多个细胞功能，通常比单靶点药物更难产生耐药性。

## 1.3 细菌的防御：耐药机制的解析与反制

细菌主要通过四种机制产生耐药性：

1. **降低药物摄取**：改变细胞膜通透性，如革兰氏阴性菌中孔蛋白的突变或缺失。
    
2. **主动外排药物**：通过**外排泵 (Efflux pumps)** 将药物泵出细胞。
    
3. **药物修饰或失活**：产生酶（如 **β-内酰胺酶**）来降解或修饰抗生素。
    
4. **靶点修饰或保护**：通过基因突变改变靶点结构，使其不再与药物结合，或产生保护蛋白来遮蔽靶点。
    

[**图 2：以 albicidin 为例解释的主要耐药机制**] 该图以新型 DNA 促旋酶抑制剂**albicidin**为例，生动地展示了细菌耐药机制的多样性和复杂性。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=NTVlYmZjZjQ4ZmRlZmNlMTJlYThmYzg3NDFhODQyOTBfZVM5dWRPREpqNFBEdUV6Nk1veWdZQzdNRUtoenY2WFRfVG9rZW46S09YZWIxSnhFb2pYN094aVVHUWNUaWJjbktlXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **描述性说明**：该图展示了针对 albicidin 的五种不同耐药机制。(A) _大肠杆菌_ 通过突变外膜转运蛋白 Tsx 阻止药物进入，或 _白色黄单胞菌_ 通过外排泵 AlbF 将其泵出。(B) _白色黄单胞菌_ 通过突变靶点（DNA 促旋酶）或产生保护蛋白 AlbG 来保护靶点。(C) _分散泛菌_ 产生肽酶 AlbD 将 albicidin 切割失活。(D) _产酸克雷伯菌_ 产生结合蛋白 AlbA 来隔离药物。(E) _鼠伤寒沙门氏菌_ 通过**基因扩增 (Gene Duplication Amplifications, GDAs)** 提高药物结合蛋白 STM3175 的产量来隔离药物。
    
- **揭示性分析**：该图的核心信息是，细菌拥有一个庞大且多层次的“防御武器库”。针对同一种药物，不同细菌可以进化出完全不同的耐药策略。这凸显了开发新药时必须全面考虑潜在的耐药通路，并强调了开发**耐药机制抑制剂**的重要性。
    

[**图 3：耐药因子抑制剂**] 该图展示了对抗耐药机制的“反制武器”。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=YmU5MDQ3NzNmMDk5ZjlmZWVlNThiNzMwOGMwMzEwMjJfcU1lN3hGQ3p4SWRva2dXbUFkalFPUUs5WW1UNWRQb2xfVG9rZW46SmNyemJUMTd1b1RZNU94MllmZWN4Q3hLbjRmXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **描述性说明**：图中展示了三类耐药因子抑制剂。(A) **β-内酰胺酶抑制剂 (β-lactamase inhibitors, BLIs)**，如 **avibactam** 和 **vaborbactam**，它们与 β-内酰胺类抗生素联用，保护后者不被降解。(B) **外排泵抑制剂 (Efflux pump inhibitors, EPIs)**，如 **PAβN**。(C) **突变频率下降 (Mutation Frequency Decline, Mfd)** 蛋白抑制剂，如 **NM102**，旨在降低细菌产生耐药突变的能力。
    
- **揭示性分析**：该图揭示了“抗生素增效剂”这一重要的研发策略。通过抑制细菌的耐药机制，可以恢复甚至增强现有抗生素的活性，是延长其临床寿命的有效手段。目前，BLIs 是该领域最成功的典范。
    

## 1.4 药物开发流程：从命中到候选的漫漫长路

[**图 4：抗菌药物开发流程示意图**] 该图是新药研发的“路线图”，清晰地展示了从早期发现到临床前开发的各个阶段和关键决策点。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=NDdkMDhiNzFmNmQ1YTJiOWYxZjY1YzM2OTk2ZGY1OWVfVVVYcHE0WldSREczVldpeThNQ05NMlVFS2VUMnJCWXBfVG9rZW46RHdhUGIzcXlIb1hsQmt4SkNrRWNpY2hmbkVnXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

**描述性说明**：该流程始于**命中化合物的发现与验证 (Hit identification & validation)**，经过**从命中到先导的转化 (Hit to Lead)**、**先导化合物优化 (Lead optimization)**，最终进入**候选化合物的确定 (Candidate profiling)**。图中红色框代表微生物学和感染模型研究，蓝色框代表药代动力学和毒理学研究。

**揭示性分析**：该图的核心在于强调抗菌药物研发是一个多参数、迭代优化的复杂过程，充满了挑战和失败的风险。

- **早期筛选**：主要通过**表型筛选 (phenotypic screening)**（即全细胞活性筛选）进行，需要评估**最小抑菌浓度 (Minimal Inhibitory Concentration, MIC)** 和**最小杀菌浓度 (Minimal Bactericidal Concentration, MBC)**。同时，必须进行初步的 **ADMET** (吸收、分布、代谢、排泄、毒性) 评估，如溶解度、血浆稳定性、细胞毒性和溶血性。
    
- **Hit-to-Lead**：需要扩大抗菌谱测试，评估耐药频率 (FoR)，并进行更深入的 ADMET 测试，如 Caco-2 通透性、CYP450 抑制等。
    
- **Lead Optimization**：需要在动物模型中评估**最大耐受剂量 (Maximum Tolerated Dose, MTD)** 和**体内药效 (in vivo efficacy)**。
    
- **Candidate Profiling**：需要进行全面的安全性评估，如遗传毒性 (Ames test)、心脏毒性 (hERG assay) 和中枢神经系统毒性等。
    

这个漫长而昂贵的过程，是导致许多项目失败和大型药企退出的主要原因。

# 第二部分：新武器的来源与发现策略

[**图 5：抗菌药物发现策略**] 该图是现代药物发现方法学的全景图，展示了三大主要技术路径。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=NDRmOGMyNWJhYzVjNzY1YWU5ZDQzM2ZkOWRjNmQwNjBfYU9FUzU1ZHN2SHN6cDR0aFVzV011R1h1TjJISFlzdlNfVG9rZW46WElKWmJRY002b0NTREx4V1FTaWNCM0xQbnJiXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **描述性说明**：该图将发现策略分为三大来源：(1) **自然 (Nature)**，包括从传统和未开发生境中分离微生物，以及利用基因组挖掘和宏基因组学；(2) **化学库 (Chemical Libraries)**，包括小分子库、药物重定位库、DNA 编码库和各种展示技术；(3) **人工智能 (Artificial Intelligence)**，利用机器学习和分子对接进行虚拟筛选和结构设计。
    
- **揭示性分析**：该图的核心是展示了现代药物发现从“偶然发现”到“理性设计”的深刻转变。**自然产物发现**正通过**基因组挖掘 (genome mining)** 和 **iChip** 等原位培养技术被重新激活。**化学合成**则通过**高通量筛选 (High-Throughput Screening, HTS)**、**DNA 编码库 (DNA-encoded libraries, DECLs)** 和**噬菌体/核糖体展示 (Phage/Ribosome display)** 等技术，能够以前所未有的规模探索化学空间。而 **AI** 则作为一种强大的赋能技术，贯穿于从靶点识别、虚拟筛选到结构优化的全过程，极大地提高了发现的效率和成功率。
    

## 2.1 源于自然的发现：经典的宝库与现代的挖掘

自然界，特别是微生物，一直是抗生素的最主要来源。

- **经典筛选**：通过大规模培养和筛选放线菌、芽孢杆菌和粘细菌等“天赋”菌株，发现了大多数经典抗生素。
    
- **拓展来源**：现代研究正转向**未开发生境**（如深海、沙漠、微生物组）和**难培养/未培养微生物**。**iChip** 技术通过在原位模拟微生物的自然生长环境，成功分离培养了之前未知的细菌，并发现了**德索巴克汀 (teixobactin)**。
    
- **基因组挖掘**：这是最具革命性的技术。通过 **antiSMASH**、**PRISM** 和 **ARTS** 等生物信息学工具，可以直接从基因组序列中识别 BGCs，预测其产物结构，并评估其新颖性。这使得研究者能够靶向性地激活沉默的 BGCs 或从宏基因组数据中发现全新的化学实体。
    

## 2.2 化学与化学生物学：从头构建新分子

- **高通量筛选 (HTS)**：利用自动化平台对数十万甚至数百万个小分子进行活性筛选。尽管在抗菌领域，由于药物渗透屏障等问题，HTS 的成功率相对较低，但它仍然是发现全新化学骨架的重要途径。**贝达喹啉 (bedaquiline)** 就是通过表型 HTS 发现的。
    
- **药物重定位 (Drug repurposing)**：筛选已批准的药物库，寻找具有抗菌活性的分子。这是一种低成本、高效率的策略。
    
- **库设计与合成**：**DNA 编码库 (DECLs)** 和**展示技术 (Display technologies)** 能够构建规模极其庞大（可达 10¹⁴）的肽或小分子库，并通过亲和筛选快速找到与靶点结合的分子。
    

## 2.3 计算科学的赋能：分子对接与人工智能

[**图 6：通过虚拟筛选/分子对接和 AI 发现的分子**] 该图展示了计算方法在药物发现中的强大威力。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=NzIzOTk2MzFkMDBmYTlmYjM2ZGFkY2M0NWEzY2VhODJfWkhRR3NuMG9KdmVkaWw0VllUbG96VzFKZWdaMGNuaDBfVG9rZW46UFBCZWJPc2VWb2g4YnR4VVB0bmM5UWl5bmtjXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **描述性说明**：图 A 展示了通过**分子对接 (Molecular Docking)** 虚拟筛选发现的 PBP2a 抑制剂 **ND-421** 和 Mfd 抑制剂 **NM102**。图 B 展示了通过**人工智能 (AI)** 深度学习模型发现的膜去极化剂 **halicin** 和 **abaucin**。
    
- **揭示性分析**：该图揭示了计算科学如何从两个层面加速药物发现。**分子对接**依赖于已知的靶点三维结构，通过计算模拟来预测小分子与靶点的结合模式和亲和力，是一种**基于结构 (structure-based)** 的理性设计方法。而 **AI** 则更进一步，它可以通过学习大量的化学结构-活性数据，构建能够预测分子活性的模型，甚至在没有明确靶点结构的情况下，从海量分子中筛选出具有特定表型（如抑制细菌生长）的候选物。**Halicin** 的发现是 AI 在该领域取得突破的标志性事件。
    

## 2.4 设计理念的革新：双效机制与先导优化

[**图 7：双效抗生素的理性设计策略**] 该图展示了三种旨在克服耐药性和增强药效的先进设计理念。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=ODhjM2FiZTE4MzE1NDk0YzYzMDBiYTlmODk4ZTc0NTVfUXNhZ05zZVlsZGx0Q3ZvczMydjV2THVkeU1tc1dxZm5fVG9rZW46SkhtZmJ3QVVzb2lJMkp4UUJzbWNmTUhFbmtkXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **描述性说明**：(A) **“特洛伊木马”策略**，将抗生素与细菌必需的营养物质（如铁载体）偶联，如 **cefiderocol**。(B) **“混合抗生素”策略**，将两个具有不同作用机制的抗生素通过一个连接子偶联，如 **CBR-2092**。(C) **“细菌 PROTAC” (bacPROTAC)** 策略，利用分子胶将靶蛋白与细菌自身的蛋白降解机器（如 ClpCP）连接，从而诱导靶蛋白的降解，如 **UdSBI-4377**。
    
- **揭示性分析**：该图的核心是展示了药物设计正从简单的“抑制剂”思维，转向更复杂的“系统干预”思维。这些策略旨在通过多种方式智取细菌：利用其自身系统进行药物递送、同时攻击多个靶点以协同增效、或劫持其蛋白稳态系统来清除靶蛋白。这些都是应对复杂耐药机制的创新性解决方案。
    

[**图 8：抗菌活性优化的研究实例**] 该图展示了从一个先导化合物出发，通过药物化学优化获得临床候选药物的四个典型案例。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=OWY3NjAxZThkMDQ2Yzc2NmY0ZWE4YzIyNjlkNzM5ZDJfeGlLaGgwSW5RbGxJd0tqZzh3WDV2S2VIbHNwcnpLcUJfVG9rZW46RUt3QmJ4WWZ3b0RDcFB4bDFkbGM0dHdobnplXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **描述性说明**：(A) 从天然产物 **albicidin** 到广谱类似物 **AF-CD-007** 的优化。(B) 从天然产物 **arylomycin** 到广谱 G- 药物 **G0775** 的改造。(C) 从天然多肽 **protegrin I** 到 G- 特效药 **murepavadin** 的进化。(D) 从 HTS 命中物 **afabicin** 到广谱 G- 药物 **fabimycin** 的改造。
    
- **揭示性分析**：该图强调了**先导化合物优化 (Lead Optimization)** 是药物研发中至关重要的一环。通过系统的**结构-活性关系 (Structure-Activity Relationship, SAR)** 研究，药物化学家可以对分子的活性、选择性、溶解度、代谢稳定性和渗透性等关键参数进行精细调整。特别是 **(B)** 和 **(D)** 的案例，展示了如何应用 **eNTRy 规则**（一组指导小分子穿透革兰氏阴性菌外膜的经验性设计原则）将一个原本只对 G+ 菌有效的分子，成功改造为对 G- 菌也有效的广谱药物。
    

[**图 9：优化支架的工具箱和方法**] 该图总结了药物化学家和生物学家用于优化先导化合物的“工具箱”。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=OWNjNTdhNTdlMTEwNGM0ZDJlOTQzODYyNTBmNGY5MWZfSHFoaTVrRzRkQ25jV3lqYTU2WVBITExmMjJNNVdNZmtfVG9rZW46S3RCR2JqUk1hb09nOU94NnY2TGNXWGt3bnpiXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **描述性说明**：左侧是**药物化学 (MedChem)** 工具箱，包括生物电子等排、理性设计、虚拟筛选和 QSAR。右侧是**生物合成 (Biosynthesis)** 工具箱，包括基因组挖掘、组合生物合成、诱变和化学酶法。图中以**四环素 (tetracyclines)** 的全合成和**放线菌素 (actagardine)** 的半合成为例。
    
- **揭示性分析**：该图强调了现代药物优化是一个多学科协作的过程。**化学家**可以通过全合成或半合成，对分子进行精细的结构修饰。**生物学家**则可以通过基因工程手段，改造生物合成途径，产生新的衍生物。二者的紧密结合，能够以最高效的方式探索化学空间，获得最佳的候选药物。
    

# 第三部分：创新药物管线全景：一场靶点导向的阅兵

[**图 10：人类医疗保健中当前抗菌药物的防线**] 该图是本部分内容的“背景板”，展示了目前临床上使用的主要抗生素类别及其作用靶点，为后续讨论的新药提供了参照系。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=ZDQzNDRkYzNkMDc3MTFkMTE4MjYxYzAxNTU4NmEzZWZfajBJUXYxV2xobVpCNFoxQk44SkdINDhGejR3RXBvd2FfVG9rZW46TzBJdWJUcUxub1lFcll4eFExSWNxOFhibmV1XzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

## 3.1 细胞壁与细胞分裂：攻破细菌的“铠甲”

[**图 11-13, 表 1**] 细胞壁是细菌（除支原体外）所特有的结构，是理想的药物靶点。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=YTI5M2QxOWM1NmRiMjBiNTQ0NzQ1Njg5MWI5MWQ1MjVfTWxKbmRFWFhHRjh2WVJ2dGgxbEk0Mm1BREZZWXhqMlBfVG9rZW46Tm9xZmJyRlNsb3BVNXJ4U0dNeGN1d2VJblpnXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

**肽聚糖 (PG) 生物合成**：

- **转糖基化 (Transglycosylation)**：**Plusbacin A₃** [**图 11A**] 是一种天然产物抑制剂。**Murgocil** [**图 11D**] 是通过表型筛选发现的 MurG 抑制剂。**Microvionin** [**图 11E**] 是一种新型的脂质 II 结合剂。
    
- **转肽 (Transpeptidation)**：**青霉素结合蛋白 (Penicillin-Binding Proteins, PBPs)** 是 β-内酰胺类抗生素的经典靶点。新型非 β-内酰胺类 PBP 抑制剂 **ETX0462** [**图 11C**] 和 **ND-421** [**图 11C**] 正在开发中，它们对 β-内酰胺酶不敏感。
    
- **脂质 II 生物合成**：**Tunicamycin** 和 **caprazamycin B** [**图 11D**] 是 MraY 的抑制剂。
    
- **自溶素 (Autolysins)**：**Corbomycin** [**图 12B**] 通过结合肽聚糖来阻断自溶素的重塑功能，代表了一种新的作用机制。
    

**壁磷壁酸 (WTA) 生物合成**：WTA 是 G+ 菌细胞壁的重要组分。**Tarocin A/B** 和 **targocil** [**图 11B**] 分别是 WTA 生物合成早期和晚期的抑制剂，它们能增强 β-内酰胺类药物对 MRSA 的活性。

**分枝菌酸与阿拉伯半乳聚糖合成 (分枝杆菌特有)**：**DprE1** 是该途径的关键酶。**BTZ043**、**PBTZ169**、**TBA-7371** 和 **OPC-167832** [**图 12A**] 都是高效的 DprE1 抑制剂，处于不同的临床开发阶段。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=YTM2YmFkMGQxZmJkZTQzMDkzOGUyNWNhYjUxYzFkMTVfb1JTY3E0bTVuMEtOMnZxRHpjWGZJdkI3ZmhYV25RQ0dfVG9rZW46SXF2aGI3VXg5b2VZRjF4QUF3emNiTVVobnNjXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

**细胞分裂**：**FtsZ** 是细菌的细胞骨架蛋白，负责形成 Z 环启动细胞分裂。**PC190723** 及其前药 **TXY436** [**图 13**] 是特异性的金黄色葡萄球菌 FtsZ 抑制剂。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=ZjA4ODYyMzA1NjNmNmZhN2FkMDQwZDhkOTE0NTRjZTZfV25XdnZEaGFjd3RXOXJlYTg1bTRjb0hjZXJCMXJNTUdfVG9rZW46TEV3cWIwellqb2JnTld4Uk9CZGNybEpQbnpoXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

## 3.2 蛋白质合成：瘫痪细菌的“工厂”

[**图 14-16, 表 2**] 核糖体是蛋白质合成的场所，其结构在细菌和真核生物间存在显著差异，是抗生素的重要靶点。

**核糖体抑制剂**：

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=ODMyNmFiNWU5YTg5NDE0M2JmYjllNzA4ODg3ZWU5ZjRfT2p1SnFlWHpjNXVNaXkzWUUxRTZuSm1raGQ5elQwYnFfVG9rZW46Rlo2cWJqNjRub0hWYmp4VWJYUmNFZ29XbmdmXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **Bottromycin C₂** 和 **myxovalargin A** [**图 14A**] 是两种天然肽类抑制剂。
    
- **Odilorhabdins** (如 **NOSO-95179** [**图 14C**]) 是一类新型的核糖体抑制剂，能诱导密码子错读。
    
- **氨酰-tRNA 合成酶 (aaRSs)**：这类酶负责为 tRNA 装载正确的氨基酸。**Mupirocin** [**图 15**] 是经典的异亮氨酰-tRNA 合成酶抑制剂。**REP8839** 和 **CRS3123** [**图 15**] 是新型的甲硫氨酰-tRNA 合成酶抑制剂。
    

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=YzcwODk0MmFlMWUwYTUyNGI0YTQyMmMwMTM2YmM4YTNfdktTNHNReFIxN3FUaWFPdGtuNTBCSmJiOG5NTEVOTzVfVG9rZW46TmhlbmJwdWRtb25oUGZ4aGpWZ2Nya1pZbjNmXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

**延伸因子 (Elongation Factors)**：**EF-Tu** 负责将氨酰-tRNA 运送至核糖体。**LFF571** [**图 16A**] 是天然产物 **GE2270A** 的半合成衍生物，是一种高效的 EF-Tu 抑制剂。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=MmM5NzgwMWIwNzcxZTU1NTA3YjY3OTRlNjhhOTNmNWZfemhmZEhhb3Z4SjF1MjdTdEFBMnZJdWFXcHVuQmZwczJfVG9rZW46WEtTNGJpcUhGb0lwZHN4RkJzS2N0QlZMbmFiXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

**肽基转移酶 (Peptide Deformylases, PDFs)**：这类酶负责切除新生肽链 N-端的甲酰基。**Actinonin** [**图 16B**] 是天然的 PDF 抑制剂，其合成类似物 **BB-83698** 和 **LBM-415** [**图 16B**] 曾进入临床研究。

## 3.3 DNA 合成：阻断遗传信息的复制

[**图 17-18, 表 3-4**]

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=OWU0ZmQyZTYwYmViZDljNTZmYzg3MTU4NTllNmEwMzFfV3o2RjgybzlDeWhlc3d3TWgzUHk2eVVGV0FZWlBFUm5fVG9rZW46S09nUWJiUksyb3BFNlR4cVVGWGN1VlRzbjdkXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

**DNA 聚合酶 (DNA Polymerase)**：**Nargenicin A1** [**图 17A**] 是天然的 Pol III 抑制剂。**Ibezapolstat** [**图 17C**] 是一种新型的 Pol IIIC 抑制剂，正在开发用于治疗艰难梭菌感染。

**DNA 滑动夹钳 (Sliding Clamp, DnaN)**：**Griselimycin** [**图 17A**] 是一种环状肽，通过结合 DnaN 来抑制 DNA 复制，对结核菌有特效。

**DNA 连接酶 (DNA Ligase)** 和 **解旋酶 (Helicase)**：**Pyridochromanone cpd 3** [**图 17D**] 和 **coumarin cpd 22** [**图 17E**] 分别是这两个靶点的抑制剂，但目前研究较少。

**拓扑异构酶 (Gyrase/Topoisomerase IV)**：这是**氟喹诺酮类**药物的经典靶点。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=MWRlZjUzYWMxNDJlYzE5ZmYzODUyMzgyM2EyNGY3YThfeWlhYzg3cUxLQ1c2SGc5MzhTc1JocUtzcE9idHlLNGpfVG9rZW46VklZVWJ6RmpWb2xyU2N4R0RuOWNDRXBMbmNjXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **新型天然产物抑制剂**：**Albicidin** [**图 18A**] 及其类似物 **cystobactamids** [**图 18C**] 是一类高效的广谱促旋酶抑制剂。**Evybactin** [**图 18C**] 是另一种新型肽类抑制剂。
    
- **新型合成抑制剂**：**新型细菌拓扑异构酶抑制剂 (Novel Bacterial Topoisomerase Inhibitors, NBTIs)** 是一类全新的合成药物，其作用机制与氟喹诺酮类不同。**Gepotidacin** [**图 18A**] 是其中的代表，已完成 III 期临床试验。**Zoliflodacin** [**图 18A**] 则是另一种处于临床后期的 NBTI，主要用于治疗淋病。
    

## 3.4 RNA 合成：沉默基因的转录

[**图 19, 表 5**] **RNA 聚合酶 (RNA Polymerase, RNAP)** 是转录过程的核心酶，也是一个成熟的靶点。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=YTljMWU4NjQwZDU5OGU1N2I0NTcwMTcwN2EwYTBhNzRfdUJ2TUhTeFBvYmlWbHZlTG9JTHEyN0VZNndkT0M0Q21fVG9rZW46UXNabWJKUEVtb1JSVkx4OXBGeGNlM0hCblEwXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **经典抑制剂**：**利福霉素 (Rifamycins)**。
    
- **新型天然产物抑制剂**：粘细菌是 RNAP 抑制剂的宝库，产生了 **sorangicin A**、**myxopyronin B** 和 **corallopyronin A** [**图 19A, B**]。其中 **corallopyronin A** 正在被开发为抗丝虫病药物。
    
- **新型合成抑制剂**：**Ridinilazole** [**图 19C**] 是一种窄谱的、非吸收的药物，通过结合 DNA 小沟槽来抑制 RNAP，对艰难梭菌有特效，已完成 III 期临床试验。
    

## 3.5 脂肪酸合成：切断膜组分的来源

[**图 20, 表 6**] 细菌的 II 型脂肪酸合成系统 (FASII) 与人体的 I 型系统 (FASI) 存在显著差异。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=M2RlZDczYWZlODM1ZTNjZWRiOTQ3ODkxZDNiYTdlZWFfVDY3UTlsem41QWhhNW1QcDBqSUlsam9BT1hjY0RuTTFfVG9rZW46RFVOVWJSdFgxbzY2aWp4UUxScmNtZWdPbmZqXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **FabI (烯酰-ACP 还原酶)**：**Afabicin** [**图 20A**] 是一种特异性的金黄色葡萄球菌 FabI 抑制剂，其前药正在进行临床试验。其衍生物 **fabimycin** [**图 8D**] 则具有广谱的 G- 活性。
    
- **FabF/H (β-酮酰-ACP 合酶)**：**Platensimycin** 和 **platencin** [**图 20C**] 是分别靶向 FabF 和 FabF/H 的天然产物。
    
- **AccAD (乙酰辅酶A 羧化酶)**：**Andrimide** [**图 20D**] 是该靶点的抑制剂。
    
- **分枝菌酸合成**：**SQ109** [**图 20D**] 是一种乙胺丁醇类似物，通过抑制分枝菌酸转运蛋白 MmpL3 发挥作用，处于临床开发阶段。
    

## 3.6 蛋白酶与降解机器：破坏蛋白质稳态

[**图 21-22, 表 7**]

**Clp 蛋白酶系统**：**ClpP** 是细菌中核心的 ATP 依赖性蛋白酶。**ADEPs (Acyldepsipeptides)** [**图 21A**] 是一类天然产物，它们不是抑制 ClpP，而是异常**激活**它，使其变成一个不受调控的蛋白降解机器，能有效杀死包括持留菌在内的细菌。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=ODlkYTUxNWI1ODcyNWIzNDY0Yjc1NWZmZDdkYWVhNGNfekZENzBBeWVJOE9rQU5hb0p3ekU0dEt3dEZFclVZRmxfVG9rZW46WWZFbGJkc0czb2kwNHB4b3ZIdGMyQU11bnZjXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

**信号肽酶 (Signal Peptidases, SPases)**：

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=MWU4YTYzMGVmNjNhZDU1MWQ1ZjNjZWQyNDIyOTBjY2FfdlhNNG9qSlVRYlRTUUwwVjhIUlc1dUI5ZW1wbmdvMW5fVG9rZW46VjNYcWJRR0Nob0tzY1V4bmVwcGNHSHVhblRoXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **SPase I**：**Arylomycins** [**图 22A**] 是天然的 SPase I 抑制剂。其合成衍生物 **G0775** [**图 22A**] 具有强大的广谱 G- 活性，是该领域的重大突破。
    
- **SPase II (LspA)**：**Globomycin** 和 **myxovirescin** [**图 22C**] 是该靶点的抑制剂。
    

## 3.7 细胞膜与生物能学：瓦解能量与屏障

[**图 23, 表 8**]

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=NTMwNDY4MjZjMGIxOTE5ZjMyNzgzMDg3MDMzMDY3OGVfQkpaVXdTOFRLdXl1Z1R3cFc2eHpMWkpZNjVsUEMxaUhfVG9rZW46R1pUdmJNMlc3b3BlVUF4eVpMTWNsNFM0bmNmXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **膜破坏剂**：**多粘菌素 (Polymyxins)**（如 colistin）是治疗多重耐药 G- 菌感染的“最后一道防线”，它们通过结合 LPS 来破坏外膜。**F365 (QPX9003)** [**图 23A**] 是新一代的多粘菌素类似物，具有更低的肾毒性。
    
- **膜去极化剂**：**Daptomycin** [**图 10**] 通过钙依赖的方式插入 G+ 菌细胞膜，导致膜去极化。**Halicin** 和 **lugdunin** [**图 23C**] 也是通过破坏质子动力势来发挥作用。
    
- **呼吸链抑制剂 (分枝杆菌)**：**Bedaquiline** [**图 10**] 是首个批准上市的 ATP 合酶抑制剂。**Telacebec (Q203)** [**图 23B**] 是细胞色素 bc₁ 复合物的抑制剂。
    
- **甲萘醌 (Menaquinone) 结合剂**：**Lysocin E** [**图 23D**] 是一种环肽，通过结合 G+ 菌呼吸链中的电子载体甲萘醌来发挥作用。
    

## 3.8 膜蛋白与组分生物发生：干扰“城墙”的建造

[**图 24-25, 表 9**] 这是当前 G- 药物研发最前沿和最热门的领域。

**脂多糖 (LPS) 生物合成与转运**：

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=YzQxNTI1MzdmOTQwMDBmYWIzMWM5ZjY1MjI4ZmUwMTlfNHhCdjhiNkdyRWJVbTdnOWJkUG1PM0pHNWlKa08yWnNfVG9rZW46UU5ramI1VVJ0b09NU1N4Q1puZGNXWW95blRoXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **LpxC**：是脂质 A 生物合成的关键酶。**ACHN-975** 和 **PF-5081090** [**图 24D**] 是高效的 LpxC 抑制剂，但因毒性问题开发受阻。
    
- **Lpt 系统**：负责将 LPS 从内膜转运至外膜。**Murepavadin** [**图 24A**] 是一种特异性靶向铜绿假单胞菌 LptD 的肽。**Zosurabalpin** [**图 24C**] 是一种新型大环肽，通过抑制鲍曼不动杆菌的 LptB₂FG 复合物发挥作用，已进入 III 期临床。**Thanatin** [**图 24B**] 则靶向 LptA/D。
    

**β-桶状蛋白组装机器 (BAM) complex**：负责将外膜蛋白正确折叠和插入。**Darobactin** [**图 25B**] 是一种 RiPP，通过模拟 β-链，“卡住”BAM 复合体的核心亚基 BamA，是首个靶向该系统的药物。

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=NDEzZDRkOWI4NmNmYmQ2NWNkNThiYWY4ZWI2OWRjNWFfUklBU1pLRWFsN2F1QktBY1gweUpNVXd2Y2ZkTEhYMTBfVG9rZW46TW9mY2JyN3FLb2pCWUZ4SEdMaWNaZURjbk9nXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

**脂蛋白转运 (Lol) 系统**：负责将脂蛋白从内膜转运至外膜。**Lolamicin** [**图 25A**] 是一种新型合成抑制剂，它能选择性地抑制致病 G- 菌的 Lol 系统，而对共生菌影响较小，代表了“微生物组友好型”抗生素的开发方向。

#### 3.9 其他新兴靶点：探索新的攻击路径

[**图 26, 表 10**]

![](https://synbiopath.feishu.cn/space/api/box/stream/download/asynccode/?code=ZWE2MmJlZDRjMmZkMmE3YjhkYmNkODQ1Yjc3NmI2NjBfYzU1MG4zbVJ2bDhFeWlaQ0NkcGpVN0dKMlZYcmZaNVlfVG9rZW46RDQ5bmJLazJ3b3VRQTB4SHVPcmM4M3NobmhkXzE3NjUxODg0MTk6MTc2NTE5MjAxOV9WNA)

- **辅因子与核苷酸合成**：**Abyssomicin C** [**图 26**] 是叶酸途径的抑制剂。**Gemcitabine** 和 **TK-666** [**图 26**] 是核苷酸合成的抑制剂。
    
- **双组分系统 (Two-component systems, TCS)**：**Signermycin B** 和 **walrycin B** [**图 26**] 分别是 WalK 和 WalR 的抑制剂，干扰 G+ 菌的细胞壁稳态调控。
    
- **孢子萌发**：**Oxadiazole cpd 2** [**图 26**] 及其衍生物能抑制艰难梭菌的孢子萌发，为治疗复发性感染提供了新策略。
    

# 第四部分：总结与展望

## **总结**

本综述系统地、深入地剖析了当前抗菌药物研发的全貌。面对日益严峻的 AMR 危机，研发范式已经发生了深刻的转变。一方面，我们对细菌生理学和耐药机制的理解达到了前所未有的深度，催生了大量新颖的、极具潜力的药物靶点，特别是在革兰氏阴性菌的外膜生物发生等前沿领域。另一方面，药物发现的技术手段也日新月异，从基因组挖掘、人工智能到创新的理性设计策略，极大地提高了发现新化学实体的效率和成功率。本文所汇编的超过 100 种处于不同研发阶段的候选药物，共同构成了一幅充满希望的创新图景，展示了全球科学界为应对这一重大公共卫生挑战所付出的巨大努力。

## **展望**

作者在文末强调，尽管取得了显著进展，但前路依然充满挑战。

- **从 G+ 到 G- 的跨越**：尽管 G+ 药物的管线相对丰富，但针对多重耐药 G- 菌的有效药物仍然是最大的未满足需求。如何设计能够有效穿透 G- 菌双层膜屏障的分子，仍是药物化学的核心挑战。
    
- **新靶点的验证与开发**：许多新兴靶点（如蛋白酶、膜生物发生系统）虽然极具吸引力，但将其转化为成功的临床药物，还需要克服诸多科学和技术障碍。
    
- **天然产物的持续重要性**：尽管合成化学和 AI 取得了巨大进步，但天然产物，特别是来自非传统来源的天然产物（如 **albicidin**, **darobactin**），仍然是发现全新化学骨架和复杂作用机制的最重要源泉。
    
- **经济模式的改革**：从根本上说，要确保持续的创新，必须改革抗生素的经济激励模式，通过“推拉”机制确保研发投入能够获得合理的回报。
    

总而言之，抗击 AMR 是一场需要化学、生物学、医学和政策制定者协同作战的持久战。本综述所展示的科学进展，为我们打赢这场战争提供了坚实的武器基础和明确的战略方向。