---
url: https://www.synbiopath.com/zh
title: "通往合成生物学之路"
description: "合成生物学研究导航与 AI 辅助平台"
captured_at: "2026-03-25T13:32:38.122Z"
---

# 通往合成生物学之路

让知识的门槛更低，让科学的探索更纯粹。

## 1\. AI 搜索

12 项

### 1.1 国外模型

5 项

[

### ChatGPT

OpenAI 最强通用模型，科研润色、逻辑推理首选。

AI通用

](https://chatgpt.com/)[

### Gemini

Google 多模态大模型，擅长长文本处理与文献分析。

Google多模态

](https://gemini.google.com/app)[

### Google AI Studio

开发者版 Gemini，可免费使用 Pro 模型，上下文窗口大。

API开发

](https://aistudio.google.com/)[

### Grok

xAI 推出的模型，具备实时推特信息流接入能力。

Twitter实时

](https://grok.com/)[

### Perplexity

结合实时联网搜索的 AI，精准提供文献出处。

搜索引用

](https://www.perplexity.ai/)

### 1.2 国内模型

5 项

[

### DeepSeek

深度求索，国产开源模型之光，代码与数学能力极强。

国产开源

](https://www.deepseek.com/)[

### 秘塔 AI 搜索

无广告的 AI 搜索引擎，结构化展示搜索结果。

国产搜索

](https://metaso.cn/)[

### 豆包

字节跳动出品，语音交互体验好，适合日常问答。

国产日常

](https://www.doubao.com/chat/)[

### Kimi

月之暗面出品，支持超长无损上下文，擅长读长论文。

国产长文本

](https://www.kimi.com/)[

### 通义千问

阿里巴巴出品，综合能力强，适合商业与文档分析。

国产阿里

](https://www.qianwen.com/)

### 1.3 提示词

2 项

### 研究性论文深度解读

AI PROMPT

扮演资深博士后，对文献图表、数据、机理进行全覆盖解读，生成学位论文水准的报告。

复制提示词

### 综述性论文结构化精读

AI PROMPT

将复杂的综述文章拆解为逻辑严密，信息详实的中文学术报告，适合快速掌握领域前沿。

复制提示词

## 2\. 学术搜索

14 项

### 2.1 学术搜索 (外文文献)

7 项

[

### Google Scholar

全球最大的学术文献索引数据库，科研必备。

文献搜索

](https://scholar.google.com/)[

### PubMed

NIH 维护的生物医学文献数据库，是生命科学领域最权威的文献检索平台。

生物医学文献

](https://pubmed.ncbi.nlm.nih.gov/)[

### Web of Science

全球最权威的学术引文数据库，科研评价与文献调研必备。

引文SCI

](https://www.webofscience.com/)[

### Semantic Scholar

AI 驱动的免费学术搜索引擎，提供精准的文献一句话总结 (TLDR) 和引用意图分析。

AI一句话总结

](https://www.semanticscholar.org/)[

### X-mol

综合性学术平台，提供文献资讯、学术社区和科研功能。

学术资讯

](https://www.x-mol.com/)[

### Bing 学术

微软必应推出的学术搜索，界面简洁，资源覆盖广。

搜索微软

](https://cn.bing.com/academic/)[

### ProQuest

全球领先的学术数据库平台，涵盖期刊论文、学位论文（PQDT）和电子书，覆盖人文社科、生物医学等多个领域。

学位论文数据库

](https://www.proquest.com/)

### 2.2 学术搜索 (中文文献)

4 项

[

### 中国知网 (CNKI)

中国最大的中文学术文献库，查硕博论文必备。

中文期刊

](https://www.cnki.net/)[

### 万方数据

综合性中文学术资源库，涵盖期刊、专利、标准。

中文数据

](https://www.wanfangdata.com.cn/)[

### 百度学术

汇聚海量中英文文献，提供免费的论文查重入口。

聚合中文

](https://xueshu.baidu.com/)[

### PubScholar（推荐）

中国科学院公益学术平台，免费获取 8000 万篇全文，集成科大讯飞翻译引擎。

免费公益

](https://pubscholar.cn/)

### 2.3 AI 文献分析与网络图谱

3 项

[

### SciSpace

读 PDF 文献的神器。自带 AI Copilot，可划词翻译、解释图表公式、总结核心结论。

PDF精读AI

](https://typeset.io/)[

### Connected Papers

极佳的文献拓扑网络可视化工具。输入一篇文献，瞬间为你梳理该领域的来龙去脉。

文献拓扑可视化

](https://www.connectedpapers.com/)[

### Elicit

你的 AI 博士后助理，能够从海量文献中自动提取实验数据、方法和结果并汇总成表格。

数据提取文献挖掘

](https://elicit.com/)

## 3\. 文献下载

10 项

### 3.1 直接下载

6 项

[

### Sci-hub（推荐）

打破学术壁垒，免费下载全球绝大多数付费论文。

免费神器

](https://www.sci-hub.st/)[

### 科研通（推荐）

文献互助平台，通过社区力量解决疑难文献下载。

互助社区

](https://www.ablesci.com/)[

### GeenMedical

国产文献下载平台，提供海量文献资源检索和下载。

下载国产

](https://www.geenmedical.com/)[

### Scidown

提供各类学术资源下载导航，无需翻墙。

导航下载

](https://www.scidown.cn/)[

### 龙猫学术

学术导航站，聚合了多个谷歌学术镜像和下载入口。

导航镜像

](http://www.6453.net/)[

### 文献部落

不仅提供文献下载，还包括电子书和专利查询。

电子书综合

](http://459.org/)

### 3.2 文献管理与阅读

4 项

[

### EndNote

全球最经典的文献管理神器，科研人员必备的论文写作助手。

文献管理老牌

](https://endnote.com/)[

### Zotero（推荐）

免费开源的文献管理工具，插件丰富，科研效率倍增。

文献管理开源

](https://www.zotero.org/)[

### 小绿鲸

专业的 SCI 文献阅读神器，支持全文翻译、笔记管理和文献分析。

文献阅读翻译

](https://www.xljsci.com/)[

### 靠岸学术

AI 驱动的文献阅读与管理平台，提供文献摘要、翻译和分析功能。

文献阅读AI

](https://www.scholaread.cn/)

## 4\. 专利相关

6 项

### 4.1 专利检索

3 项

[

### 国家知识产权局

中国官方专利检索系统，数据最权威。

官方中国

](https://pss-system.cponline.cnipa.gov.cn/conventionalSearch)[

### SooPAT

强大的专利搜索引擎，界面友好，支持全文检索。

搜索工具

](https://www.soopat.com/)[

### Espacenet

欧洲专利局提供的全球专利检索数据库。

全球欧洲

](https://worldwide.espacenet.com/)

### 4.2 专利下载

3 项

[

### DrugFuture (CN)

中国专利全文免费下载工具。

下载中国

](https://www.drugfuture.com/cnpat/cn_patent.asp)[

### DrugFuture (US)

美国专利全文免费下载工具。

下载美国

](https://www.drugfuture.com/uspat/us_patent.asp)[

### DrugFuture (EU)

欧洲专利全文免费下载工具。

下载欧洲

](https://www.drugfuture.com/eppat/patent.asp)

## 5\. 天然产物

22 项

### 5.1 波谱分析与结构鉴定

8 项

[

### 微谱数据

国内知名的核磁共振碳谱数据库。

NMR图谱

](http://www.nmrdata.com/new/#/)[

### NP-MRD

天然产物磁共振数据库，用于结构鉴定。

NMR结构鉴定

](http://www.np-mrd.org/)[

### GNPS

全球天然产物社会分子网络，质谱数据分析神器。

质谱分子网络

](http://gnps.ucsd.edu/)[

### SIRIUS

顶尖的 AI 质谱分析工具，集成 CSI:FingerID，从高分辨 MS/MS 数据中直接预测代谢物结构。

质谱AI

](https://bio.informatik.uni-jena.de/software/sirius/)[

### SMART

基于 NMR 的天然产物结构快速去重与识别工具。

NMR去重

](http://smart.ucsd.edu/classic)[

### DeepSAT

深度学习辅助的谱图分析工具。

AI深度学习

](https://github.com/cure-lab/DeepSAT)[

### DP5

用于辅助核磁共振结构解析的概率预测工具。

NMR预测

](https://github.com/Goodman-lab/DP5)[

### Pep2Path

将肽类质谱数据关联到基因组生物合成基因簇。

肽基因组

](https://pep2path.sourceforge.net/)

### 5.2 专属天然产物库

6 项

[

### CMNPD

综合海洋天然产物数据库，挖掘海洋新颖骨架与高活性分子的权威平台。

海洋新颖骨架

](https://www.cmnpd.org/)[

### TCMSP

中药系统药理学数据库，收录海量中药有效成分及其靶点信息。

中药网络药理学

](https://www.91tcmsp.com/#/home)[

### KNApSAcK

经典的物种-代谢物关系数据库，精准查询特定植物或微生物中的天然产物。

植物物种关系

](http://www.knapsackfamily.com/KNApSAcK/)[

### NPEdia

专注于天然产物及其生物活性的百科全书。

百科活性

](http://www.cbrg.riken.jp/npedia/?LANG=en)[

### NP Atlas

微生物天然产物图谱，提供结构与来源信息。

微生物图谱

](https://www.npatlas.org/)[

### DBAASP

抗菌肽结构与活性数据库。

抗菌肽活性

](http://dbaasp.org/)

### 5.3 综合化学数据库

4 项

[

### PubChem

NIH 提供的全球最大化学分子数据库。

NIH权威

](https://pubchem.ncbi.nlm.nih.gov/)[

### ChemSpider

皇家化学会提供的免费化学结构数据库。

RSC结构

](http://www.chemspider.com/)[

### MoNA

北美质谱库，全球最大的非靶向代谢组学和天然产物免费质谱数据库之一。

质谱代谢组学

](https://mona.fiehnlab.ucdavis.edu/)[

### ChEBI

生物学关注的小分子化学实体数据库。

EBI小分子

](https://www.ebi.ac.uk/chebi/)

### 5.4 性质预测与辅助计算

4 项

[

### SwissADME

极简直观的成药性、水溶性、透脑性及 ADME 预测工具，科研绘图首选。

ADME成药性

](http://www.swissadme.ch/)[

### SwissTargetPrediction

基于反向药效团匹配原理，预测天然产物小分子在人体内的未知靶点蛋白。

靶点预测药物设计

](http://www.swisstargetprediction.ch/)[

### ADMETlab 3.0

全面评估药物的吸收、分布、代谢、排泄和毒性。

药物设计ADMET

](https://admetlab3.scbdd.com/)[

### ChemCalc

化学计算器，快速计算分子式和同位素分布。

工具计算

](https://www.chemcalc.org/mf-finder)

## 6\. 基因工具

36 项

### 6.1 基因序列综合分析

12 项

[

### Neurosnap

基于 AI 的生物学综合分析平台。

AI综合

](https://neurosnap.ai/)[

### EBI Tools

EBI 提供的各类生物信息学在线工具集合。

EBI工具箱

](https://www.ebi.ac.uk/Tools/msa/clustalo)[

### NCBI

美国国家生物技术信息中心，生命科学核心门户。

权威数据库

](https://www.ncbi.nlm.nih.gov/)[

### 国家基因组科学数据中心

中国国家基因组科学数据中心，提供海量基因组数据资源。

中国基因组

](https://ngdc.cncb.ac.cn/?lang=zh)[

### Ensembl

极其强大的真核生物基因组浏览器和注释数据库，查找转录本首选。

基因组真核生物

](https://www.ensembl.org/)[

### TBtool

强大的生物序列批量处理与分析工具。

软件批量

](https://apps.microsoft.com/detail/9nvr9lb426p2)[

### GTEx Portal

基因组织表达数据库，研究基因在不同组织中的表达模式。

基因表达组织

](https://www.gtexportal.org/home/)[

### GeneCards

人类基因综合数据库，整合了基因功能、表达、变异等多源信息。

人类基因综合

](https://www.genecards.org/)[

### IMP

整合药用植物组学平台，收录1007个高质量植物基因组、8亿+基因及2158个转录组样本，提供基因注释、序列、功能、分布和表达等分析。

植物基因组

](https://www.bic.ac.cn/IMP/#/)[

### SGD

酿酒酵母基因组数据库，提供酵母基因序列、注释、功能、蛋白质等详细信息。

酵母基因组

](https://www.yeastgenome.org/)[

### Nicomics

华中农业大学开发的烟草基因组数据库，提供烟草基因组、转录组等组学数据。

烟草基因组

](http://lifenglab.hzau.edu.cn/Nicomics/)[

### DOE JGI

美国能源部联合基因组研究所，提供 IMG、GOLD、Genome Portal 等微生物基因组数据库和 BBTools 序列处理工具。

基因组微生物

](https://jgi.doe.gov/)

### 6.2 结构域与同源分析

5 项

[

### eggNOG-Mapper

基于 eggNOG 数据库的快速基因功能注释工具，支持 GO、KEGG、COG 等注释，比传统 BLAST 方法快 2-15 倍。

功能注释同源分析

](https://github.com/eggnogdb/eggnog-mapper)[

### InterPro

蛋白质家族、结构域和功能位点分类数据库。

结构域家族

](https://www.ebi.ac.uk/interpro/)[

### HMMER

基于隐马尔可夫模型的序列同源性搜索工具。

同源性HMM

](https://www.ebi.ac.uk/Tools/hmmer/)[

### CD Search

NCBI 保守结构域数据库搜索。

NCBI结构域

](https://www.ncbi.nlm.nih.gov/Structure/cdd/wrpsb.cgi)[

### SCOP2

蛋白质结构分类数据库。

结构分类

](http://scop.mrc-lmb.cam.ac.uk/)

### 6.3 序列比对与进化树

5 项

[

### Clustal Omega

快速且精确的多序列比对工具。

比对MSA

](https://www.ebi.ac.uk/jdispatcher/msa/clustalo?stype=protein)[

### MAFFT

高性能的多序列比对程序。

比对高性能

](https://mafft.cbrc.jp/alignment/server/index.html)[

### Jalview

用于编辑、可视化和分析多序列比对的软件。

可视化编辑

](https://www.jalview.org/)[

### MEGA

分子进化遗传分析软件，构建进化树必备。

进化树遗传

](https://www.megasoftware.net/)[

### iTOL

在线系统发育树展示与注释工具。

进化树在线

](http://itol.embl.de/login.cgi)

### 6.4 基因组与引物设计

4 项

[

### Circos

用于可视化数据和信息的圆形图表软件，常用于绘制基因组。

可视化基因组

](https://circos.ca/)[

### Oligo Calc

寡核苷酸性质计算器。

引物计算

](http://oligocalc.eu/)[

### IDT OligoAnalyzer

IDT 提供的专业引物分析工具，分析二聚体和发卡结构。

引物分析

](https://www.idtdna.com/calc/analyzer)[

### Augustus

真核生物基因及内含子注释预测工具。

基因预测真核

](https://bioinf.uni-greifswald.de/augustus/submission.php)

### 6.5 基因编辑与 CRISPR

5 项

[

### CHOPCHOP

哈佛大学开发，极易上手的 CRISPR/Cas (Cas9, Cpf1 等) sgRNA 设计与脱靶效应预测工具。

CRISPRsgRNA设计

](https://chopchop.cbu.uib.no/)[

### CRISPOR

目前引用率最高的「金标准」工具之一，评估脱靶效应并直接输出克隆所需的引物序列。

金标准引物

](http://crispor.tefor.net/)[

### Benchling CRISPR

工业界最受欢迎的综合质粒与序列设计平台，内置极其强大的 CRISPR 导向 RNA 设计模块。

综合平台工业界

](https://www.benchling.com/crispr/)[

### Cas-OFFinder

超高速搜索 CRISPR/Cas 脱靶位点，允许自定义错配数量（mismatches）和序列凸起（bulges）。

脱靶分析算法

](http://www.rgenome.net/cas-offinder/)[

### Synthego Design

全球领先的 CRISPR 公司提供的免费在线设计工具，特别擅长设计多重基因敲除实验。

免费商业基因敲除

](https://design.synthego.com/)

### 6.6 质粒与克隆构建

2 项

[

### Addgene

全球最大的非盈利性质粒共享库，查找验证过的质粒图谱、序列和骨架的必去之地。

质粒数据库

](https://www.addgene.org/)[

### NEBuilder

NEB 官方提供的无缝克隆（Gibson Assembly / HiFi）引物设计神器。

无缝克隆引物设计

](https://nebuilder.neb.com/)

### 6.7 表达元件与 RNA 分析

3 项

[

### RBS Calculator

Salis Lab 开发的合成生物学核心工具，极其精准地预测和设计核糖体结合位点（RBS），控制翻译速率。

RBS翻译控制

](https://salislab.net/software/)[

### BPROM

经典的细菌启动子（Promoter）预测工具，寻找转录起始位点和 -10/-35 区域。

启动子细菌

](http://www.softberry.com/berry.phtml?topic=bprom&group=programs&subgroup=gfindb)[

### RNAfold

预测 RNA 二级结构的标准工具，设计核糖开关（Riboswitch）或优化 mRNA 稳定性必备。

RNA二级结构

](http://rna.tbi.univie.ac.at/cgi-bin/RNAWebSuite/RNAfold.cgi)

## 7\. 蛋白工具

70 项

### 7.1 蛋白综合分析平台

5 项

[

### Tamarind

基于 AI 的蛋白质结构与功能分析平台。

AI蛋白

](https://app.tamarind.bio/app)[

### Novopro

纽谱生物提供的多肽与蛋白在线工具集。

工具集多肽

](https://novopro.cn/tools/)[

### Expasy

瑞士生物信息学研究所提供的蛋白质组学服务器。

蛋白质组瑞士

](https://www.expasy.org/)[

### DTU Health Tech

丹麦技术大学提供的生物信息学工具集，包括膜蛋白预测等。

膜蛋白工具集

](https://services.healthtech.dtu.dk/)[

### CD-HIT

高效的蛋白质序列聚类工具，用于构建非冗余数据库。

序列聚类非冗余

](https://www.bioinformatics.org/cd-hit/)

### 7.2 蛋白结构预测

14 项

[

### AlphaFold 3

Google DeepMind 最新一代蛋白质结构预测模型。

结构预测AI

](https://deepmind.google/science/alphafold/alphafold-server/)[

### ColabFold

无需本地算力，在浏览器中极速运行 AlphaFold2 预测蛋白及复合物结构的开源神器。

结构预测Colab

](https://colab.research.google.com/github/sokrypton/ColabFold/blob/main/AlphaFold2.ipynb)[

### Chai-1

前沿的分子结构预测大模型，在预测蛋白质与小分子配体、核酸复合物方面具有顶尖精度。

AI大模型

](https://app.chaidiscovery.com/)[

### Swiss-Model

全自动蛋白质同源建模服务器。

同源建模结构

](https://swissmodel.expasy.org/)[

### RoseTTAFold

华盛顿大学 Baker 实验室开发的蛋白结构预测工具。

结构预测BakerLab

](https://github.com/RosettaCommons/RoseTTAFold)[

### ESMFold

Meta 开发的基于语言模型的快速蛋白结构预测。

Meta速度快

](https://github.com/facebookresearch/esm)[

### Protenix

专业的蛋白分析、结构预测与设计服务器。

分析设计

](https://protenix-server.com/)[

### I-TASSER

迭代线程组装优化算法，蛋白质结构预测的经典工具。

结构预测同源建模

](https://aideepmed.com/I-TASSER/)[

### QUARK

从头预测蛋白质结构的工具，适用于无同源模板的蛋白。

从头预测自由建模

](https://aideepmed.com/QUARK/)[

### Robetta

Baker 实验室的蛋白结构预测服务器，支持单链和复合物预测。

结构预测BakerLab

](https://robetta.bakerlab.org/)[

### AWSEM

基于原子可移动性的蛋白质结构预测工具。

结构预测分子动力学

](https://awsem-md.org/)[

### DSSP

蛋白质二级结构预测和分类工具。

二级结构螺旋

](https://pdb-redo.eu/dssp)[

### Dali server

基于蛋白质结构比对的网络服务器，用于搜索结构相似的蛋白质。

结构比对同源

](http://ekhidna2.biocenter.helsinki.fi/dali/)[

### 3D-Beacons Network

整合多个蛋白质结构预测服务的网络平台。

结构预测整合

](https://www.ebi.ac.uk/pdbe/pdbe-kb/3dbeacons/)

### 7.3 分子对接

12 项

[

### AutoDock

分子模拟与对接领域的标准软件。

对接模拟

](https://autodock.scripps.edu/)[

### AutoDock Vina

速度更快、精度更高的开源分子对接引擎。

对接开源

](https://vina.scripps.edu/)[

### HDock

混合算法蛋白-蛋白、蛋白-DNA/RNA 对接服务器。

对接蛋白互作

](http://hdock.phys.hust.edu.cn/)[

### DiffDock

基于扩散模型的分子对接工具。

AI扩散模型

](https://github.com/gcorso/DiffDock)[

### ClusPro

波士顿大学开发的蛋白-蛋白对接服务器。

蛋白对接互作

](https://cluspro.bu.edu/login.php)[

### CABS-dock

华沙大学开发的多肽-蛋白对接工具。

多肽对接柔性对接

](https://biocomp.chem.uw.edu.pl/CABSdock/)[

### HPEPDOCK

华中科技大学开发的多肽-蛋白对接服务器。

多肽对接柔性

](http://huanglab.phys.hust.edu.cn/hpepdock/)[

### Flexible Peptide Docking

Meiler 实验室开发的多肽柔性对接服务器。

柔性对接多肽

](http://servers.meilerlab.org/index.php/servers)[

### RPBS

巴黎第七大学的生物分子模拟门户。

分子模拟工具集

](https://mobyle.rpbs.univ-paris-diderot.fr/cgi-bin/portal.py#welcome)[

### CavityPlus

北京大学开发的蛋白空腔识别与药物结合位点预测工具。

空腔识别结合位点

](http://www.pkumdl.cn:8000/cavityplus/#/)[

### ChemSAR

化合物 SAR 分析与分子对接在线工具。

SAR对接

](http://chemsar.scbdd.com/home/index/)[

### DeepScreening

基于深度学习的虚拟筛选工具。

虚拟筛选深度学习

](http://deepscreening.xielab.net/index)

### 7.4 蛋白序列与结构库

8 项

[

### UniProt

通用蛋白质资源库，包含序列与功能信息。

数据库序列

](https://www.uniprot.org/)[

### NCBI Protein

NCBI 维护的蛋白质序列数据库。

NCBI序列

](https://www.ncbi.nlm.nih.gov/protein)[

### AlphaFold DB

AlphaFold 预测的数亿个蛋白结构数据库。

数据库结构

](https://alphafold.ebi.ac.uk/)[

### RCSB PDB

蛋白质数据银行，存储实验测定的生物大分子晶体结构。

晶体结构

](https://www.rcsb.org/)[

### STRING

全球最大、最权威的已知和预测蛋白质-蛋白质相互作用（PPI）网络数据库。

PPI相互作用

](https://string-db.org/)[

### IntAct

EBI 提供的分子相互作用数据库。

蛋白质相互作用PPI

](https://www.ebi.ac.uk/intact/home)[

### The Human Protein Atlas

人类蛋白质图谱计划，提供蛋白质在组织、细胞中的表达信息。

人类蛋白组织表达

](https://www.proteinatlas.org/search)[

### OPM

Orientations of Proteins in Membranes 数据库。

膜蛋白数据库

](https://opm.phar.umich.edu/)

### 7.5 酶性质与定位预测

8 项

[

### ExpOptimizer

密码子优化工具，提高异源表达水平。

表达优化

](https://www.novopro.cn/tools/codon-optimization.html)[

### Protein-Sol

预测蛋白质序列的可溶性，帮助湿实验人员提前避开形成包涵体的雷区。

溶解度包涵体

](https://protein-sol.manchester.ac.uk/)[

### SignalP

预测蛋白质信号肽及其切割位点。

信号肽定位

](https://services.healthtech.dtu.dk/services/SignalP-6.0/)[

### DeepTMHMM

基于深度学习的跨膜螺旋预测工具。

跨膜深度学习

](https://dtu.biolib.com/DeepTMHMM)[

### Rare Codon Analyzer

分析基因序列中的稀有密码子。

密码子分析

](http://www.detaibio.com/tools/rare-codon-analyzer.html)[

### PPSP

蛋白质磷酸化位点预测工具。

磷酸化预测

](http://ppsp.biocuckoo.org/)[

### GPS-SUMO

蛋白质SUMO化修饰位点预测工具。

SUMO化预测

](https://sumo.biocuckoo.cn/index.php)[

### MAI-TargetFisher

基于多尺度相互作用的多功能靶点预测工具。

靶点预测多尺度

](https://bailab.siais.shanghaitech.edu.cn/mai-targetfisher)

### 7.6 酶挖掘与功能注释

5 项

[

### EFI-EST

酶相似性工具，用于构建序列相似性网络 (SSN)。

SSN酶

](https://efi.igb.illinois.edu/efi-est/)[

### FoldSeek

像 BLAST 一样快速的蛋白质结构搜索工具。

结构搜索快速

](https://search.foldseek.com/search)[

### CLEAN

基于对比学习的酶功能注释工具。

AI功能注释

](https://clean.platform.ibiofoundry.illinois.edu/configuration)[

### EnzymeMiner

基于深度学习的酶挖掘工具。

酶挖掘AI

](https://loschmidt.chemi.muni.cz/enzymeminer/)[

### MPEPE

基于机器学习的蛋白酶活性与稳定性预测工具。

蛋白酶活性预测

](http://www.elabcaas.cn/pird/mpepe)

### 7.7 蛋白设计与改造

11 项

[

### RFdiffusion

Baker 实验室开发的基于扩散模型的蛋白质生成工具，能够从零设计全新结构的蛋白骨架。

扩散模型生成式AI

](https://github.com/RosettaCommons/RFdiffusion)[

### ODesign

基于深度学习的蛋白亲和力优化设计工具。

亲和力优化

](https://odesign1.github.io/)[

### BindCraft

基于深度学习的蛋白-配体结合设计工具，能够从头设计结合特定小分子的蛋白质。

配体结合AI

](https://github.com/martinpacesa/BindCraft)[

### CACLENS

基于 AI 的蛋白质活性与稳定性优化平台。

AI优化

](https://ai.caclens.com/)[

### CATNIP

基于机器学习的蛋白-配体结合亲和力预测工具。

结合预测机器学习

](https://catnip.cheme.cmu.edu/)[

### EZSpecificity

酶特异性优化工具，用于设计和优化酶的底物特异性。

酶特异性优化

](https://doi.org/10.5281/zenodo.17129896)[

### MPEK

蛋白酶热稳定性与动力学参数预测工具。

蛋白酶热稳定性

](http://mathtc.nscc-tj.cn/mpek)[

### LigandMPNN

ProteinMPNN 的重磅升级版，支持包含小分子配体和金属离子的蛋白质序列设计。

序列设计配体

](https://github.com/dauparas/LigandMPNN)[

### ProteinMPNN

根据蛋白质结构设计序列的深度学习模型。

序列设计AI

](https://github.com/dauparas/ProteinMPNN)[

### FoldX

用于计算突变对蛋白稳定性影响的工具。

稳定性突变

](https://foldxsuite.crg.eu/)[

### SaProt

西湖大学开发的结构感知蛋白质语言模型。

语言模型结构感知

](https://github.com/westlake-repl/SaProt)

### 7.8 三维结构可视化

7 项

[

### PyMOL

业界最经典的蛋白质分子可视化软件，功能强大，支持高质量论文级别图片渲染。

可视化经典

](https://www.pymol.org/)[

### UCSF ChimeraX

新一代分子可视化软件，支持高性能渲染和增强现实，是 UCSF Chimera 的升级版本。

可视化3D

](https://www.cgl.ucsf.edu/chimerax/)[

### Mol\*

极其流畅的 Web 端 3D 大分子结构可视化工具，也是 PDB 数据库官方指定的渲染引擎。

3D可视化

](https://molstar.org/viewer/)[

### 3Dmol.js

轻量级的基于 WebGL 的分子可视化库，支持在浏览器中极速渲染结构。

WebGL渲染

](https://3dmol.csb.pitt.edu/)[

### ESPript

蛋白质序列比对结果的美化输出工具，生成出版级比对图。

序列比对可视化

](https://espript.ibcp.fr/ESPript/cgi-bin/ESPript.cgi)[

### WebLogo

创建序列 LOGO 图的可视化工具，展示保守性区域。

序列LOGO可视化

](https://weblogo.threeplusone.com/create.cgi)[

### colornames

UCSF Chimera 的颜色命名命令文档。

可视化颜色

](https://www.cgl.ucsf.edu/chimerax/docs/user/commands/colornames.html)

## 8\. 生物合成与代谢

31 项

### 8.1 BGC 挖掘与网络分析

12 项

[

### antiSMASH

最权威的次级代谢基因簇挖掘与分析工具。

挖掘BGC

](https://antismash.secondarymetabolites.org/#!/start)[

### DeepBGC

基于深度学习的 BGC 挖掘工具，能够发现传统规则无法覆盖的新型未知「暗物质」基因簇。

AI深度学习

](https://github.com/Merck/deepbgc)[

### BiG-SCAPE

计算基因簇相似性并构建序列相似性网络 (GCF)。

BGC网络

](https://github.com/medema-group/BiG-SCAPE)[

### ARTS

抗生素抗性靶点搜寻工具，用于发现新抗生素。

抗生素抗性

](https://arts.ziemertlab.com/)[

### SMBP

次级代谢产物生物合成挖掘平台，提供基因簇预测与分析功能。

BGC挖掘

](http://www.secondarymetabolites.org/)[

### FunGeneClusterS

真菌次级代谢基因簇挖掘与分析工具。

真菌BGC

](https://fungiminions.shinyapps.io/FunGeneClusterS/)[

### FunARTS

针对真菌的抗生素抗性靶点搜寻工具。

真菌抗性

](https://funarts.ziemertlab.com/)[

### FunBGCs

真菌生物合成基因簇功能分析与挖掘工具。

真菌BGC

](https://staffweb1.cityu.edu.hk/ymatsuda/funbgcs/funbgcs.html)[

### 2ndFind

次级代谢产物生物合成基因查找工具。

搜索代谢

](https://biosyn.nih.go.jp/2ndfind/)[

### BAGEL4

专门用于挖掘细菌素和 RiPPs 的工具。

细菌素RiPPs

](http://bagel4.molgenrug.nl/)[

### RiPPMiner

用于核糖体合成后修饰肽的自动挖掘与分析。

RiPPs修饰肽

](http://www.nii.ac.in/rippminer.html)[

### GenoCGLLM

植物基因簇分析与比较基因组学平台。

基因簇植物

](https://www.bic.ac.cn/GenoCGLLM/)

### 8.2 结构域与底物预测

9 项

[

### PKS/NRPS Analysis

经典的 PKS 和 NRPS 结构域分析网站。

PKSNRPS

](http://nrps.igs.umaryland.edu/)[

### NaPDoS2

基于系统发育关系快速检测和分类次级代谢结构域。

分类KS结构域

](https://npdomainseeker.sdsc.edu/napdos2/)[

### NRPSpredictor2

预测 NRPS 腺苷化结构域的底物特异性。

NRPS底物

](https://github.com/roettig/NRPSpredictor2)[

### SBSPKS

用于聚酮合酶（PKS）结构与序列分析。

PKS结构域

](http://www.nii.ac.in/~pksdb/sbspks/homepage.html)[

### Cagecat

真菌次级代谢基因簇分析与可视化平台。

真菌BGC

](https://cagecat.bioinformatics.nl/)[

### LassoPred

Lasso 肽结构域预测工具。

Lasso结构域

](https://lassopred.accre.vanderbilt.edu/)[

### PARAS

PKS/NRPS 腺苷化结构域底物特异性预测工具。

NRPS底物预测

](https://paras.bioinformatics.nl/submit)[

### DeepAden

基于深度学习的 NRPS 腺苷化结构域底物预测工具。

NRPS深度学习

](https://deepnp.site/)[

### NRPSTransformer

NRPS/PKS 功能分析与产物预测平台。

NRPSPKS

](http://www.nrpstransformer.cn/)

### 8.3 代谢通路设计与逆向合成

4 项

[

### KEGG

京都基因与基因组百科全书，代谢通路（Pathway）分析的绝对权威和核心底层数据库。

代谢通路权威

](https://www.kegg.jp/)[

### RetroBioCat

极其强大的生物催化与计算机辅助逆向合成平台，通过酶促反应设计目标分子的合成路线。

逆向合成生物催化

](https://retrobiocat.com/)[

### BioCyc

高度精选的代谢通路数据库（包含经典的 EcoCyc），提供极其详尽的单一物种代谢网络模型。

代谢网络EcoCyc

](https://biocyc.org/)[

### Selenzyme

为人工设计的代谢途径自动匹配、打分并筛选最佳候选酶序列的在线服务器。

酶筛选代谢工程

](http://selenzyme.synbiochem.co.uk/)

### 8.4 BGC 与代谢模型数据库

6 项

[

### MIBiG

已知功能的生物合成基因簇标准数据库。

数据库BGC

](https://mibig.secondarymetabolites.org/)[

### antiSMASH Database

antiSMASH 预测的生物合成基因簇数据库。

BGC数据库

](https://antismash-db.secondarymetabolites.org/)[

### DoBISCUIT

日本 NITE 开发的 PKS/NRPS 生物合成基因簇数据库。

PKSNRPS

](https://www.nite.go.jp/nbrc/pks/)[

### IMG-ABC

JGI 开发的生物合成基因簇图谱。

JGI图谱

](https://img.jgi.doe.gov/abc)[

### BiGG Models

全球最大的基因组尺度代谢网络模型（GEMs）精选数据库。

GEMs代谢模型

](http://bigg.ucsd.edu/)[

### Natural Products Discovery Center

天然产物发现中心数据库。

天然产物数据库

](https://npdc.rc.ufl.edu/home)

## 9\. 生物医药

9 项

### 9.1 药物数据库

5 项

[

### DrugBank

最权威的药物靶点与药物相互作用数据库，提供全面的药物化学和药理学信息。

药物靶点

](https://go.drugbank.com/)[

### ChEMBL

EBI 提供的具有药物样性质的生物活性分子数据库。

药物活性

](https://www.ebi.ac.uk/chembl/)[

### BindingDB

蛋白质-配体结合亲和力数据库，是药物发现的重要资源。

结合亲和力配体

](https://www.bindingdb.org/rwd/bind/index.jsp)[

### IUPHAR

国际药理学协会提供的药物靶点和受体数据库。

药理学受体

](https://www.guidetopharmacology.org/)[

### ClinPGx

临床药物基因组学数据库，提供药物基因组学临床应用指南。

临床基因组学

](https://www.clinpgx.org/)

### 9.2 癌症组学分析

4 项

[

### TCGA

癌症基因组图谱计划，提供33种癌症的多组学数据。

癌症多组学

](https://portal.gdc.cancer.gov/)[

### cBioPortal

癌症基因组学数据可视化和分析平台。

癌症可视化

](https://www.cbioportal.org/)[

### GEPIA

北京大学开发的癌症基因表达分析平台。

基因表达癌症

](http://gepia.cancer-pku.cn/)[

### LINCS

LINCS 细胞信号网络数据库，记录基因扰动后的表达变化。

细胞信号基因扰动

](https://lincsportal.ccs.miami.edu/dcic-portal/)

## 10\. 实验耗材与试剂

10 项

### 10.1 化学试剂与耗材

7 项

[

### ChemicalBook

化学信息检索平台，提供 CAS 号、物理化学性质、供应商信息查询。

化学CAS

](https://www.chemicalbook.com/)[

### 阿拉丁

综合性化学试剂供应商。

试剂化学

](https://www.aladdin-e.com/)[

### 麦克林

国产化学试剂平台。

试剂化学

](https://www.macklin.cn/)[

### 国药

国药集团化学试剂。

试剂化学

](https://www.reagent.com.cn/)[

### 索莱宝

国产知名生物试剂品牌。

试剂国产

](http://www.solarbio.com/)[

### Sigma-Aldrich

全球领先的化学试剂供应商。

试剂化学

](https://www.sigmaaldrich.com/)[

### Thermo Fisher Scientific

全球最大的科学服务供应商。

试剂仪器

](https://www.thermofisher.cn/)

### 10.2 生物试剂与服务

2 项

[

### TargetMol

全球领先的生物试剂供应商。

试剂化合物

](https://www.targetmol.cn/)[

### 淼灵

国产生物试剂平台。

质粒国产

](https://www.miaolingbio.com/)

### 10.3 微生物菌种资源

1 项

[

### 中国普通微生物菌种保藏管理中心

国内最大的微生物菌种保藏机构。

菌种保藏

](https://www.cgmcc.net/)

## 11\. 科研绘图

8 项

[

### ImageGP 2.0

在线科研绘图工具，简单粘贴数据即可出图。

在线简单

](https://www.bic.ac.cn/BIC/)[

### Hiplot

国产综合性科研绘图平台，支持多种图表类型。

绘图国产

](https://hiplot.com.cn/home/index.en.html)[

### Figdraw

国产在线科研绘图工具，提供丰富的素材模板。

绘图国产

](https://www.figdraw.com/#/)[

### BioRender

生物医学领域的「Canva」，拥有海量精美素材。

绘图素材

](https://www.biorender.com/)[

### BioDraw

另一款生物绘图工具，适合绘制通路图。

绘图通路

](https://biodraws.com/)[

### Plottie

新兴的科研绘图平台。

绘图艺术

](https://plottie.art/)[

### Eryphonix

免费的科研绘图工具，提供丰富的生物科学模板和素材。

绘图免费

](https://reseachbub.org/)[

### EVenn

在线韦恩图绘制工具，支持多组数据交集分析。

韦恩图交集

](https://www.bic.ac.cn/EVenn/)

[鲁ICP备2026011208号](https://beian.miit.gov.cn/)